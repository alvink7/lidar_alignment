...
def ground_stability(pcd, params, label, trials):
    ...
    print("  [%s] fitting ground plane %d times (RAW segment_plane):" %
          (label, trials))
    normals = []
    ...
    # what the CONSTRAINED (horizontal) search picks — this is what the
    # alignment actually uses now
    from alignment_core import _find_horizontal_plane
    n2, inl2 = _find_horizontal_plane(
        icp, params.GROUND_DIST_THRESHOLD, params.GROUND_PLANE_ITERS, params)
    ang2 = np.degrees(np.arccos(np.clip(abs(n2[2]), -1, 1)))
    tag2 = "HORIZONTAL(floor)" if ang2 < 20 else \
           ("STILL VERTICAL(wall!)" if ang2 > 70 else "TILTED")
    print("     -> CONSTRAINED search picks: n=[%.3f %.3f %.3f]  %.1f deg  %s\n" %
          (n2[0], n2[1], n2[2], ang2, tag2))
    return normals[0]
